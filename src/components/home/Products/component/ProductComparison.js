...
function ProductComparison() {
    ...
    const handleProductSelect = (event) => {
        ...
        const selectedProduct = products.find((products) => products.id === selectedProductId);
        if (selectedProduct) {
            if (selectedProducts.length < 2) {
                setSelectedProducts([...selectedProducts, selectedProduct]);
            } else {
                // Handle error or notification if the user tries to select more than two products
                console.log('You can only select two products for comparison.');
            }
        }
    };
    ...
    const renderSelectedProducts = () => {
        return selectedProducts.map((products) => (
            <div key={products.id} className="border p-4 mb-4">
                <h3>{products.name}</h3>
            </div>
        ));
    };
    return (
        <div class="overflow-x-auto mt-10">
            <div className='bg-green-100 mx-auto py-2 px-2 rounded-sm shadow-sm flex justify-between'>
                <div class="relative">
                    <select onChange={handleProductSelect} class="block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline">
